...
def create_two_data_frames(df_labels_a1):
    print("df_labels_a1 \n",df_labels_a1.columns)
    ### Putcome 1 dataset per participnt
    # Select the first 10 columns
    p5_labels_subset = df_labels_a1[['Elapsed_Time', 'Off seat\n (0 -On seat ,1- yes off seat ) \n',
       'Hyperactive or Restless', 'Hyperactive Behaviour Type',
       'Annotator2_HyperactiveBehaviourType_Second_Coder',
       'Second Hyperactive Behaviour Type ',]]#df_labels_a1.iloc[:, :10]
    p5_labels_subset.columns = ['Elapsed_Time', 'Off seat\n (0 -On seat ,1- yes off seat ) \n',
       'Hyperactive or Restless', 'Hyperactive Behaviour Type',
       'Annotator2_HyperactiveBehaviourType_Second_Coder',
       'Second Hyperactive Behaviour Type ']
    # Drop the first row
        ...
        if os.path.isdir(folder_path) and foldername.startswith('A'):
            ...
            if p6_file_paths and labels_file_path:
                try:
                    ...
                    labels_df = pd.read_csv(labels_file_path,skiprows=1)
                    ...
                    for p6_file_path in p6_file_paths:
                        ...
                        upsampled_result_labels.to_csv(os.path.join(root_session + "syncedfiles2/", new_result_name), index=False)
...
SESSION_ID = "Session1_DC_04_29_2023/" # Eg: Session5_DC_06_03_2023/
